# GeoEPred
GeoEPred is a geometry-aware multimodal learning framework for the prediction of secreted effectors in Gram-negative bacteria. The key idea behind GeoEPred is to reformulate effector prediction from the perspective of joint sequence-structure representation.Specifically, GeoEPred first applies an attention mechanism to the embedding vectors generated by the pretrained protein language model ESM-2, so as to further extract contextual semantic information from protein sequences. It then employs a geometric vector perceptron (GVP) to encode local geometric patterns and residue-level spatial relationships from protein structures, enabling the joint modeling of semantic and conformational features. Building on this, GeoEPred further integrates an auxiliary contrastive learning objective with a tokenization-based multi-head attention mechanism to enhance cross-modal alignment between sequence and structure and to model fine-grained multimodal interactions, thereby improving the identification of key effector-related patterns. 
...
## **Train model*
You can train the GeoEPred model by running `train.py` for cross-validation. 
```shell
python train.py \
